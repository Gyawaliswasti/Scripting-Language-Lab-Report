...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $rate = isset($_POST["rate"]) ? $_POST["rate"] : "<br>";
    $time = isset($_POST["time"]) ? $_POST["time"] : "<br>";
...
<body>
    ...
    <span><h2>Simple Interest Calculator</h2></span>
        ...
        <label  for="principle">Principle:</label>
          <input type="number" name="principle"   value="<?php echo isset($principle) ? ($principle) : ''; ?>" /><br><br>
       

        <label  for="rate">Rate of Interest:</label>
          <input type="number" name="rate"   value="<?php echo isset($rate) ? ($rate) : ''; ?>"/><br><br>
      

        <label  for="time">Time</label>
          <input type="number" name="time"   value="<?php echo isset($time) ? ($time) : ''; ?>"/><br><br>
</div>  
